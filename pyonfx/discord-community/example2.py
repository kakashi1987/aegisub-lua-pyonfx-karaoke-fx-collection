...
def romaji(line, l):
	...
	for syl in Utils.all_non_empty(line.syls):
		...
		l.start_time = line.start_time + 25*syl.i - delay - 80 # Remove 80 to start_time to let leadin finish a little bit earlier than the main effect of the first syllable
		l.end_time = line.start_time + syl.start_time
		l.dur = l.end_time - l.start_time

		l.text = "{\\an5\\move(%.3f,%.3f,%.3f,%.3f,0,%d)\\blur2\\t(0,%d,\\blur0)\\fad(%d,0)}%s" % (
			syl.center + math.cos(syl.i / 2)*off_x, syl.middle + math.sin(syl.i / 4)*off_y, syl.center, syl.middle, delay, # Move
			delay, # transform
			delay, # fad
			syl.text)
	...
	for syl in Utils.all_non_empty(line.syls):
		...
		if syl.inline_fx == "m2":
			on_inline_effect_2 = "\\t(0,%d,\\frz%.3f)\\t(%d,%d,\\frz0)" % (
				l.dur/4, random.uniform(-40, 40), l.dur/4, l.dur)

		l.text = "{\\an5\\pos(%.3f,%.3f)%s\\t(0,80,\\fscx105\\fscy105\\1c%s\\3c%s)\\t(80,%d,\\fscx100\\fscy100\\1c%s\\3c%s)}%s" % (
			syl.center, syl.middle, # pos
			on_inline_effect_2, # random frz depending on inline effect
			c1, c3, # transform 1
			l.dur - 80, line.styleref.color1, line.styleref.color3, # transform 2
			syl.text)
		...
		jump_width = line.syls[syl.i+1].center - syl.center if syl.i != len(line.syls) - 1 else syl.width
	...
	for syl in Utils.all_non_empty(line.syls):
		...
		l.text = "{\\an5\\move(%.3f,%.3f,%.3f,%.3f,%d,%d)\\t(%d,%d,\\blur2)\\fad(0,%d)}%s" % (
			syl.center, syl.middle, syl.center + math.cos(syl.i / 2)*off_x, syl.middle + math.sin(syl.i / 4)*off_y, l.dur-delay, l.dur, # Move
			l.dur-delay, l.dur, # transform
			delay, # fad
			syl.text)
...
def translation(line, l):
	...
	for i in range(1, int(line.width/80)):
		...
		l.text = "{\\an5\\pos(%.3f,%.3f)\\fad(%d,%d)\\clip(%d,%d,%d,%d)%s}%s" % (
			line.center, line.middle, line.leadin/2, line.leadout/2, clip[0], clip[1], clip[2], clip[3], colors, line.text)
		io.write_line(l)
