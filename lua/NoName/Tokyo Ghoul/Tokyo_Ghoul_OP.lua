...
function kara (line, l)
	...
	if line.style == "tg_OP_romanji" or line.style == "tg_OP_romanji2" or line.style == "tg_OP_romanji_L" or line.style == "tg_OP_romanji_R" or line.style == "tg_OP_kanji" or line.style == "tg_OP_kanji_L" or line.style == "tg_OP_kanji_R" or line.style == "tg_OP_kanji2" then
	...
	if line.style == "tg_OP_TL" or line.style == "tg_OP_TL_L" or line.style == "tg_OP_TL_R" then
			...
			for ci, char in pairs(line.chars) do
				...
				if char.text~="" or char.text~=" " then
					...
					for s, e, i, n in utils.frames(line.start_time-line.infade, line.start_time+char.start_time, max_frame) do
						...
						local rx = char.center + math.randomsteps(-0.5,0.5,0.1)
						local ry = char.middle + math.randomsteps(-0.6,0.6,0.2)
						...
						l.text=string.format("{\\an5\\bord0\shad1\\blur0.6\\alpha%s\\1c%s\\4c%s\\pos(%.3f,%.3f)}%s"
						,alpha,main_color,shad_color,rx,ry,char.text)
				...
				if char.text~="" or char.text~=" " then
					...
					for s, e, i, n in utils.frames(line.start_time-line.infade, line.start_time+char.start_time, max_frame) do
						...
						l.text=string.format("{\\an5\\bord2\\blur5\\1a&HFF&\\3a%s\\3c%s\\pos(%.3f,%.3f)}%s"
						,alpha,bord_color,char.center,char.middle,char.text)
				...
				if char.text~="" or char.text~=" " then
					...
					for s, e, i, n in utils.frames(line.start_time+char.start_time, line.end_time+line.outfade, max_frame) do
						...
						if i>(n-math.ceil(fad/max_frame)) then
							alpha = utils.interpolate(u_out/max, "&H00&", "&HFF&")
							x1 = char.center + math.randomsteps(-0.5,0.5,0.1)
							y1 = char.middle + math.randomsteps(-0.6,0.6,0.2)
							x2 = char.center+of_x_dist:get()+math.randomsteps(-6,6,2)
							y2 = char.middle+of_y_dist:get()+math.randomsteps(-6,6,3)
							x = utils.interpolate(u_out/max, x1, x2)
							y = utils.interpolate(u_out/max, y1, y2)
							u_out=u_out+1
						else
							alpha_array = {"&H00&" , "&H00&" , "&H00&" , "&H00&" ,"&H00&" , "&H32&" , "&H00&" }
							alpha=alpha_array[math.random(1,table.getn(alpha_array))]
							x = char.center + math.randomsteps(-0.5,0.5,0.1)
							y = char.middle + math.randomsteps(-0.6,0.6,0.2)
						end
						
						l.text=string.format("{\\an5\\bord0\\shad1\\blur0.6\\alpha%s\\1c%s\\4c%s\\pos(%.3f,%.3f)}%s"
						,alpha,main_color,shad_color,x,y,char.text)
				...
				if char.text~="" or char.text~=" " then
					...
					for s, e, i, n in utils.frames(line.start_time+char.start_time, line.end_time+line.outfade, max_frame) do
						...
						l.text=string.format("{\\an5\\bord2\\blur5\\1a&HFF&\\3a%s\\3c%s\\pos(%.3f,%.3f)}%s"
						,alpha,bord_color,x,y,char.text)
