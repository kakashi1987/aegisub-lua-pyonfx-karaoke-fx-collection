...
function kara(line, l)
	...
	if line.style == "AD_ED2_TL" or line.style == "AD_ED2_TL_L" or line.style== "AD_ED2_TL_R" then
		
		for ci,char in ipairs(line.chars) do
			
			if line.actor == "green" then
				color = "&HBBD26B&"
				s_color = "&HBBD26B&"
			end
			...
			if line.actor == "both" then
				color = utils.interpolate(char.i/#line.chars, "&HBBD26B&", "&H6F9DE1&")
				s_color = utils.interpolate(char.i/#line.chars, "&H6F9DE1&", "&HBBD26B&")
			end
			...
			if char.text~="" then
				...
				l.text = string.format("{\\an5\\b1\\bord3\\shad2\\blur1.8\\fscx100\\fscy100\\fad(250,250)\\4a&H64&\\1c%s\\3c%s\\4c%s\\pos(%.3f, %.3f)}%s"
			...
			if char.text~="" then
				...
				l.text = string.format("{\\an5\\b1\\bord0.1\\shad0\\blur3\\fscx100\\fscy100\\fad(250,250)\\3a&H64&\\1c&HFFFFFF&\\3c&H000000&\\pos(%.3f, %.3f)}%s"
		...
		for si,syl in ipairs(line.syls) do
			...
			if syl.text~="" then
				...
				local square = shape.rectangle(7,7)
				...
				for s, e, i, n in utils.frames(line.start_time+syl.start_time, line.start_time+syl.end_time, max_frame) do
					...
					l.text = string.format("{\\an5\\p1\\bord0\\shad0\\blur0.8\\frz%d\\t(\\frz%d)\\fscx10\\fscy10\\t(0,%d,\\fscx140\\fscy140)\\t(%d,%d,\\fscx30\\fscy30)\\t(%d,%d,\\fscx140\\fscy140)\\t(%d,%d,\\fscx30\\fscy30)\\1c%s\\fad(100,250)\\move(%.3f, %.3f, %.3f, %.3f)}%s"
					,frz,frz+math.randomsteps(-180, 180, 360),pul_1, pul_1,pul_2 ,pul_2,pul_3, pul_3,pul_4,color,x,y,x-50,y+math.random(-15,15),square)
