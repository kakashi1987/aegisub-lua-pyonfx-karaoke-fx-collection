...
ass.load("K:\\AmberdrakeSubs\\Hatsuyuki\\Black Bullet\\13\\ED\\Black_Bullet_ED13_[Timed][Edited].ass")
local function infade_bord(ctx, ms, line)
	for si, syl in ipairs(line.syls) do
		if ms >= line.start_time-line.infade+50*syl.i-50 and ms < line.start_time-line.infade+50*syl.i+200 and syl.text ~= "" then
			...
			tctx:set_source(g2d.create_color(252/255, 190/255, 59/255, 1))
			tctx:stroke()
...
local function fx_bord1(ctx, ms, line)
	if line.style == "ED_romanji" or line.style == "ED_kanji" then
		for si, syl in ipairs(line.syls) do
			if ms >= syl.start_time and ms < syl.end_time and syl.text ~= "" then
				local pct = math.inrange(syl.start_time, syl.end_time, ms)
				local ang = math.interpolate(1, 180, math.inrange(syl.start_time, syl.end_time, ms))
				text = g2d.create_image("RGBA", syl.width+20, syl.height+20)
				local tctx = text:get_context()
				tctx:path_add_text(10, 10, syl.text, ass.unpack_font(line.styleref))
				tctx:set_line_width(5)
				g2du.path_transform(tctx,
				function(x, y)
					local xdef, ydef = 0, 4*math.sin(x*pct/6)
					return x + xdef, y + ydef
				end)
				tctx:set_source(g2d.create_color(255/255, 252/255, 19/255, 1))
				tctx:stroke()
				text = g2d.create_pattern(text:convolute(g2du.create_motion_blur_kernel(10*math.sin(math.rad(ang)), ang)))
				text:set_matrix(g2d.create_matrix()
...
local function fx_bord2(ctx, ms, line)
	if line.style == "ED_romanji" or line.style == "ED_kanji" then
		for si, syl in ipairs(line.syls) do
			if ms >= syl.start_time and ms < syl.end_time and syl.text ~= "" then
				local pct = math.inrange(syl.start_time, syl.end_time, ms)
	...
	if line.style == "ED_romanji" or line.style == "ED_kanji" then
		for si, syl in ipairs(line.syls) do
			if ms >= syl.start_time and ms < syl.end_time and syl.text ~= "" then
				...
				:add_color(-gradient_width + gradient_progress, 1, 1, 1, 1)
				:add_color(-gradient_width/2 + gradient_progress, 255/255, 252/255, 19/255, 1)
...
function GetFrame(frame, frame_i)
	...
	for li, line in ipairs(lines) do
		...
		fx_bord1(ctx, ms, line)
		fx_bord2(ctx, ms, line)
		fx(ctx, ms, line)
