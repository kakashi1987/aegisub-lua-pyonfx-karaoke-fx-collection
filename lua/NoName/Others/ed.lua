	...
	for i = 1, line.kara.n do
		...
		if inline=="part" then
		 --particulas
			...
			for u=0,2 do
				...
				tem2= tem1+z+line.duration/1.5 -- tiempo 2
				tf= interpolate(z/bucle, tem1, tem2) -- interpolacion para los tiempos
				
				pos2y=(y+modiy)+(s*math.sin(math.rad(pos2x)))
				 
				l.start_time= tf-bucle
				l.end_time= tf+1700
				 --borde
				 l.text="{"..an(5)..pos(pos2x,pos2y)..blur(4)..alfa(1,'FF')..color(3,'FFDE15')..alpha('80')..fscx(150)..fscy(150)..bord(1)..t(alpha('FF')..fscx(0)..fscy(0)).."}"..p(1,'m 0 0 l 1 0 1 1 0 1')
				 subs.append(l)
				 --relleno
		...
		--Silabas estaticas
			...
			if longitud >0 then
				...
				modang1=math.random(180)
				modang2=rand(30,200)
				--random para el tiempo
				randt=math.random(300,700)
				for r=0,1 do
					if syl.duration >2000 then
						bucle2=370
					else
						bucle2=270
					end	
					for tt=0,bucle2 do	
						tem1= line.start_time+syl.start_time -- tiempo 1
						tem2= tem1+tt+syl.duration/1.5 -- tiempo 2
						tf= interpolate(tt/bucle2, tem1, tem2) -- inter pa los tiempos
						
						l=table.copy(line)
																	
						l.start_time= tf-200
						l.end_time= tf+300
						pos3x=(x+mod1x)+(modamp*math.cos(math.rad(4*(tt+modang1))))
						pos3y=(y+mod1x)+(modamp*math.cos(math.rad(5*(tt+modang1))))
						 --borde
						 l.text="{"..an(5)..pos(pos3x,pos3y)..blur(4)..alfa(1,'FF')..color(3,'FFDE15')..alpha('80')..fscx(150)..fscy(150)..bord(1)..t(alpha('FF')..fscxy(0)).."}"..p(1,'m 0 0 l 1 0 1 1 0 1')
						 subs.append(l)
						 --relleno
						 l.text="{"..an(5)..pos(pos3x,pos3y)..be(1)..color(1,line.styleref.color3)..fscx(200)..fscy(200)..bord(0)..t(alpha('FF')..fscxy(0)).."}"..p(1,'m 0 0 l 1 0 1 1 0 1')
						 subs.append(l)
						subs.append(l)
						...
						l.start_time= tf-100
						l.end_time= tf+400
						pos3x=(x+mod2x)+(modamp2*math.cos(math.rad(4*((tt+modang2)+130))))
						pos3y=(y+mod2x)+(modamp2*math.cos(math.rad(5*((tt+modang2)+130))))
						 --borde
						 l.text="{"..an(5)..pos(pos3x,pos3y)..blur(4)..alfa(1,'FF')..color(3,'FFDE15')..alpha('80')..fscx(150)..fscy(150)..bord(1)..t(alpha('FF')..fscxy(0)).."}"..p(1,'m 0 0 l 1 0 1 1 0 1')
						 subs.append(l)
						 --relleno
