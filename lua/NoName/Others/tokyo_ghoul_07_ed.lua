			...
			if char.text~="" then
				...
				for s, e, i, n in utils.frames(line.start_time-line.infade+15*char.i-15, line.start_time+char.start_time, max_frame) do
					...
					if i<math.ceil(fad1/max_frame) then
						...
						shad_alpha = utils.interpolate(u_in1/max1, "&HFF&", "&H96&")
						u_in1 = u_in1 + 1
					else
						alpha="&H00&"
						shad_alpha="&H96&"
					...
					l.text=string.format("{\\an5\\bord2\\shad4\\blur2\\1a%s\\3a%s\\4a%s\\1c&HFFFFFF&\\3c&H000000&\\4c&H000000&\\fscx100\\fscy100\\pos(%.3f,%.3f)}%s"
					,alpha,alpha,shad_alpha,sx,sy,char.text)
			...
			if char.text~="" then
				...
				for s, e, i, n in utils.frames(line.start_time+char.end_time, line.end_time+line.outfade+15*char.i+15, max_frame) do
					...
					if i > (n-math.ceil(fad1/max_frame)) then
						...
						shad_alpha = utils.interpolate(u_out1/max1, "&H96&", "&HFF&")
						u_out1 = u_out1 + 1
					else
						alpha="&H00&"
						shad_alpha="&H96&"
		...
		for si,syl in ipairs(line.syls) do
			...
			if syl.text~="" then
				...
				for s, e, i, n in utils.frames(line.start_time+syl.start_time, line.start_time+syl.end_time, max_frame) do
					...
					l.text=string.format("{\\an7\\p4\\bord2\\shad4\\blur2\\4a&H96&\\1c&HFFFFFF&\\3c&H000000&\\4c&H000000&\\fscx100\\fscy100\\pos(%.3f,%.3f)}%s"
					,sx,sy,deform)
