...
function do_fx(subs, meta, line)
	...
	for j = 1, maxj do --text hilight char 1
		for i = 1, line.kara.n do 
			local syl = line.kara[i]
			local charx = table.copy(syl)	
			local pos_x = 0
			for c in unicode.chars(syl.text_stripped) do 
				...
				if syl.duration > 0 then
					...
					l.text=string.format("{\\an5\\1vc(&HCB372D&,&HCB372D&,&HFFFFFF&,&HFFFFFF&)\\3c&HCB372D&\\alpha&H55&\\bord%d\\shad0\\blur%d\\fscx100\\fscy100\\move(%d,%d,%d,%d,%d,%d)\\t(0,%d,\\fscx%d\\fscy%d)\\t(%d,%d,\\frz%d\\fscx0\\fscy0)\\%s\\%s\\fad(0,500)}%s",1*ratio,2*ratio,line.left+syl.left+x_width/2+pos_x,line.middle,line.left+syl.left+x_width/2+pos_x,line.middle+j*5,syl.duration+200,syl.duration+800,syl.duration,100*ratio,100*ratio,syl.duration,syl.duration+800,360+50*j,autotags(200,'rndx4','rndx8',syl.duration),autotags(100,'rnd4','rnd8',syl.duration),charx.text)
					l.layer=1
					subs.append(l)
					pos_x=pos_x+x_width
				end
	...
	for i = 1, line.kara.n do --text hilight syl 2
		...
		if syl.duration > 0 then
			...
			l.text=string.format("{\\an5\\1vc(&HCB372D&,&HCB372D&,&HFFFFFF&,&HFFFFFF&)\\3c&HCB372D&\\bord%d\\shad0\\blur%d\\fscx%d\\fscy%d\\pos(%d,%d)\\t(0,%d,\\fscx%d\\fscy%d\\1vc(&HFFFFFF&,&HFFFFFF&,&HFFFFFF&,&HFFFFFF&))\\%s\\%s\\jitter(2,2,3,3,100)}%s",1.6*ratio,3*ratio,120*ratio,120*ratio,syl.center+line.left,line.middle,syl.duration,100*ratio,100*ratio,autotags(200,'rndx4','rndx8',syl.duration),autotags(100,'rnd4','rnd8',syl.duration),syl.text_stripped)
			l.layer=1
			subs.append(l)
		end
	end
...
script_name = "Aegisub Karaoke Effect [35]"
