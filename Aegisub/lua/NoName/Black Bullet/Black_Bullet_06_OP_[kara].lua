...
function kara (line, l)
	...
	for si, syl in ipairs(line.syls) do
		...
		if syl.i == 1 and line.style == "OP_romanji_L" or line.style == "OP_romanji_R"then
			...
			if syl.text~="" then
				...
				for s, e, i, n in utils.frames(line.start_time-1000, line.start_time+1000, max_frame) do
					...
					if line.style == "OP_romanji_L" then
						xpos = utils.interpolate(step, line.left, line.right) + 2
						ypos = line.middle + 25
					end
					if line.style == "OP_romanji_R" then
						xpos = utils.interpolate(step, line.right, line.left) + 2
						ypos = line.middle - 25
					end
			...
			if syl.text~="" then
				...
				for s, e, i, n in utils.frames(line.start_time-1000, line.start_time+1000, max_frame) do
					...
					if line.style == "OP_romanji_L" then
						xpos = utils.interpolate(step, line.left, line.right)
						ypos = line.middle + 25
					end
					if line.style == "OP_romanji_R" then
						xpos = utils.interpolate(step, line.right, line.left)
						ypos = line.middle - 25
					end
		...
		if syl.text~="" then
			...
			for s, e, i, n in utils.frames(line.start_time-750, line.start_time-50*ord, max_frame) do
				...
				local bord = utils.interpolate(step, 3, 0.5)
				...
				l.text=string.format("{\\p4\\an7\\blur3\\bord%.2f\\shad0\\fscx100\\fscy100\\frz%.3f\\1c&HFFFFFF&\\3c%s\\pos(%.3f, %.3f)\\org(%.3f,%.3f)}%s"
				,bord,rotation,color2,xpos,ypos,orgx,orgy,deform)
		...
		if syl.text~="" then
			...
			l.start_time = line.start_time-50*ord
