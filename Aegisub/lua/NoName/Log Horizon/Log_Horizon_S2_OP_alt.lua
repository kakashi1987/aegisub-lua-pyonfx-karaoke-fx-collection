		...
		if line.style == "OP_Romanji" then
			...
			if char.text~="" and char.text~=" " then
				...
				for s, e, i, n in utils.frames(line.start_time-line.infade+15*char.i, line.end_time+15*char.i-line.outfade, max_frame) do
					...
					if i < math.ceil(infad/max_frame) then
						main_alpha = utils.interpolate(u_in/inmax, "&HFF&", "&H00&")
						bord_alpha = utils.interpolate(u_in/inmax, "&HFF&", "&H96&")
					...
					elseif s >= (line.start_time+char.start_time) and s <= (line.start_time + char.start_time + char.duration) then
						...
						x, y = char.left + math.random(-2, 2), char.top + math.random(-2, 2)
					elseif i > (n-math.ceil(outfad/max_frame)) then
						main_alpha = utils.interpolate(u_out/outmax, "&H00&", "&HFF&")
						bord_alpha = utils.interpolate(u_out/outmax, "&H64&", "&HFF&")
					...
					l.text = string.format("{\\an7\\p4\\blur1.2\\bord2\\fscx%d\\fscy%d\\1c&%s\\3c%s\\fad(0,250)\\1a%s\\3a%s&\\move(%.3f, %.3f, %.3f, %.3f)}%s"
					,sc,sc,random_primary_color,random_secondary_color,main_alpha,bord_alpha,xpos,ypos,xmov,ymov,deform)
			...
			if char.text~="" and char.text~=" " then
				...
				for s, e, i, n in utils.frames(line.start_time-line.infade+15*char.i, line.end_time+15*char.i-line.outfade, max_frame) do
					...
					l.text = string.format("{\\an7\\p4\\blur0.4\\bord5\\fscx%.3f\\fscy%.3f\\alpha%s\\1c&HFFFFFF&\\3c&H000000&\\pos(%.3f, %.3f)}%s"
