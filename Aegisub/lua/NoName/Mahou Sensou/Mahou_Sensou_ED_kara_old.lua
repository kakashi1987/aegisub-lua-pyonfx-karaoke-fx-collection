	...
	for si,syl in ipairs(line.syls) do
		...
		if syl.text~="" then
			if line.style=="ED_playing" then
				...
				l.end_time=line.end_time+50*syl.i-line.outfade/8
		...
		if syl.text~="" then
			if line.style=="ED_grid" then
				...
				l.start_time=line.start_time+50*syl.i+line.outfade/8
				l.end_time=line.start_time+syl.start_time
		...
		if syl.text~="" then
			if line.style=="ED_main_romanji" then
				--main fadein
				l.layer=1
				...
				l.text=string.format("{\\an5\\bord0\\blur0.6\\1c&HFFFFFF&\\fad(150,0)\\pos(%.3f,%.3f)}%s"
				,syl.center,syl.middle,syl.text)
			...
			if line.style=="ED_main_romanji" then
				...
				l.text=string.format("{\\an5\\bord1\\blur3\\1a&HFF&\\3c&H000000&\\fad(150,0)\\pos(%.3f,%.3f)}%s"
				,syl.center,syl.middle,syl.text)
		...
		if syl.text~="" then
			if line.style=="ED_main_romanji" then
				...
				l.text=string.format("{\\an5\\bord1\\blur3\\fscx110\\fscy110\\t(\\fscy170)\\1a&HFF&\\3c&H000000&\\fad(0,%d)\\move(%.3f,%.3f,%.3f,%.3f)}%s"
				,syl.duration,syl.center,syl.middle,syl.center,syl.middle-3,syl.text)
		...
		if syl.text~="" then
			if line.style=="ED_main_romanji" then
				...
				for pi, pixel in ipairs(convert.text_to_pixels(syl.text, line.styleref) ) do
					...
					random_end_time=(math.random(350,800))+(50*syl.i)
					
					l.start_time=line.start_time+syl.start_time
					l.end_time=line.start_time+syl.end_time+random_end_time
					
					x = syl.left + pixel.x
					y = syl.top + pixel.y
					
					random_curve_xmove_value=math.random(-5,-3)
					random_curve_xmove_amplifier=(math.random(1,3))+syl.i
					random_curve_xmove=random_curve_xmove_value*random_curve_xmove_amplifier
					random_curve_ymove=math.random(-15,15)
					random_curve_angle1=(syl.i-10)*(math.random(-17,-8))
					random_curve_angle2=(syl.i-10)math.random(-7,-1)
					random_curve_radius1=math.random(1,3)
					random_curve_radius2=math.random(2,4)
					
					l.text=string.format("{\\an5\\bord1\\blur3\\fscx100\\fscy100\\1c&HFFFFFF&\\3c&HFFFFFF&\\mover(%.3f,%.3f,%.3f,%.3f,%d,%d,%d,%d)\\p1}m 0 0 l 1 0 1 1 0 1"
					,x,y,x+random_curve_xmove,y+random_curve_ymove,random_curve_angle1,random_curve_angle2,random_curve_radius1,random_curve_radius2)
