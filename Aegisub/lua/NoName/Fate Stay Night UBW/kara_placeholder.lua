	...
	if line.actor == "fx1" then
		...
		for si,syl in ipairs(line.syls) do
			...
			if syl.text~="" then
				...
				local bord_color_dist = utils.distributor({"&HF8FF16&", "&HC74609&"})
				
				maxj = math.random(1,2)
				for j = 1, maxj, 1 do
					max_frame = 10
					...
					for s, e, i, n in utils.frames(rs, re, max_frame) do
						...
						l.text=string.format("{\\p1\\an8\\bord0.2\\shad0\\blur2\\fscx300\\fscy300\\t(\\fscx0\\fscy0)\\1c%s\\3c%s\\pos(%.3f,%.3f)}m 0 0 l 1 0 1 1 0 1"
